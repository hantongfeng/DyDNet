...
class EarlyStopping(object):
    ...
    def save_checkpoint(self, model: nn.Module):
        ...
        if self.model_name == ['DyDNet']:
            additional_data = {
                "projection_layer": model.projection_layer.state_dict(),  
                "freq_transformers": [layer.state_dict() for layer in model.freq_transformers] 
            }
            torch.save(additional_data, f"{self.save_model_path}_additional")
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None):
        ...
        if self.model_name == ['DyDNet']:
            additional_data = torch.load(f"{self.save_model_path}_additional", map_location=map_location)

            model.projection_layer.load_state_dict(additional_data['projection_layer'])

            for layer, state_dict in zip(model.freq_transformers, additional_data['freq_transformers']):
