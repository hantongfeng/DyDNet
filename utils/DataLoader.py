...
# =====================================
# MODIFICATION: Fixed transductive/inductive setting
# Date: 2025-10
# Description: Separated seen and unseen samples properly
# =====================================
...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    # # NEW: Transductive test set (seen samples only)
    # validation and test data (excluding edges with new nodes)
    observed_val_mask = np.logical_and(val_mask, ~edge_contains_new_node_mask)
    observed_test_mask = np.logical_and(test_mask, ~edge_contains_new_node_mask)
    
    val_data = Data(src_node_ids=src_node_ids[observed_val_mask], dst_node_ids=dst_node_ids[observed_val_mask],
                    node_interact_times=node_interact_times[observed_val_mask], edge_ids=edge_ids[observed_val_mask], labels=labels[observed_val_mask])

    test_data = Data(src_node_ids=src_node_ids[observed_test_mask], dst_node_ids=dst_node_ids[observed_test_mask],
                     node_interact_times=node_interact_times[observed_test_mask], edge_ids=edge_ids[observed_test_mask], labels=labels[observed_test_mask])
